...
export default function NoteTopNav({
  ...
  return (
    ...
    >
      <TabsList className="flex h-[50px] w-full items-stretch justify-center rounded-none bg-transparent p-0">
        {NOTE_TABS.map((tab) => (
          <TabsTrigger
            key={tab.value}
            value={tab.value}
            className="flex h-full flex-1 items-center justify-center rounded-none border-b-2 border-transparent bg-transparent px-4 py-0 text-center text-base font-medium leading-6 text-slate-600 shadow-none ring-0 transition-colors hover:text-foreground data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:font-semibold data-[state=active]:text-foreground data-[state=active]:shadow-none"
          >
            {tab.label}
          </TabsTrigger>
        ))}
